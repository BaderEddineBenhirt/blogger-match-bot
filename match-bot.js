...
const ACCESS_TOKEN = process.env.ACCESS_TOKEN; // Generated by GitHub Actions
    ...
    $('.AY_Match').each((index, element) => {
      try {
        ...
        if (matchLinkElement.length > 0) {
          ...
          if (href) {
            // Fix URL construction - avoid doubling
            if (href.startsWith('http')) {
              matchLink = href; // Already complete URL
            } else if (href.startsWith('/')) {
              matchLink = `https://www.kooraliive.com${href}`; // Add domain
            } else {
              matchLink = `https://www.kooraliive.com/${href}`; // Add domain and slash
            }
          }
        }
...
async function extractIframeFromMatch(matchUrl) {
  try {
    ...
    // Look for the exact iframe pattern you showed me
    let iframe = null;
    
    // First, try to find the exact pattern: <p><iframe allowfullscreen='true' frameborder='0' height='500px' scrolling='1' src='...' width='100%'></iframe></p>
    const iframeSelectors = [
      'p iframe[allowfullscreen][height="500px"]',  // Your exact pattern
      'iframe[allowfullscreen][height="500px"]',    // Without the p tag
      'iframe[src*="alkoora.live"]',                // Specific domain from your example
      'iframe[src*="albaplayer"]',                  // Specific player from your example
      '.entry-content iframe',                      // Standard content area
      '.entry iframe',
      '#the-post iframe',
      'article iframe',
      'iframe[allowfullscreen]',                    // Any fullscreen iframe
      'iframe'                                      // Fallback
    ];
    ...
    for (const selector of iframeSelectors) {
      ...
      foundIframes.each((index, element) => {
        ...
        // Skip ads and unwanted iframes, prioritize video players
        if (src && 
            !src.includes('ads') && 
          ...
          // Extract all original attributes
          iframe = {
    ...
    // Enhanced debugging - show all iframes found
    if (!iframe) {
      ...
      // Also try to find any video-related content
...
async function createPost(match) {
  try {
    ...
    let playerSection;
    if (iframeData) {
      // Create the exact iframe HTML you want embedded
      playerSection = `
