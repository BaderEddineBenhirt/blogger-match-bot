...
async function extractIframeFromMatch(matchUrl) {
  try {
    ...
    // Look for iframes in the content area specifically
    let iframe = null;
    
    // Try different selectors for iframes - prioritizing those in content areas
    const iframeSelectors = [
      '.entry-content iframe',  // Most likely location
      '.entry iframe',
      '#the-post iframe',
      'article iframe',
      '.post-content iframe',
      'iframe[src*="alkoora"]',  // Based on your example
      'iframe[src*="albaplayer"]',
      'iframe[src*="player"]',
      'iframe[src*="stream"]',
      'iframe[src*="live"]',
      'iframe[allowfullscreen]',  // iframes with allowfullscreen are usually players
      'iframe[height="500px"]',   // Based on your example
    ...
    for (const selector of iframeSelectors) {
      ...
      foundIframes.each((index, element) => {
        ...
        // Skip ads and unwanted iframes
        if (src && 
            !src.includes('ads') && 
            !src.includes('advertisement') && 
            !src.includes('banner') &&
            !src.includes('aqle3.com') &&  // Skip ad scripts
            !src.includes('bvtpk.com') &&  // Skip ad scripts
            src.length > 10) {  // Skip very short/empty srcs
      ...
      if (iframe) break; // Break out of selector loop if iframe found
    }
    
    // If no iframe found, log available iframes for debugging
    if (!iframe) {
...
async function createPost(match) {
  try {
    ...
    let playerSection;
    if (iframeData) {
      // Enhanced iframe section with beautiful styling
      playerSection = `
        <div id="match-player" style="text-align: center; margin: 30px 0; padding: 25px; background: linear-gradient(135deg, #1a1a1a 0%, #2d2d2d 100%); border-radius: 15px; box-shadow: 0 6px 12px rgba(0,0,0,0.3);">
          ...
          <div class="player-container" style="position: relative; width: 100%; height: 0; padding-bottom: 56.25%; background: #000; border-radius: 10px; overflow: hidden; box-shadow: 0 4px 8px rgba(0,0,0,0.5);">
            <iframe 
              src="${iframeData.src}" 
...
async function createMatchPosts() {
  try {
    ...
    for (const match of matches) {
      ...
      // Much longer wait between requests to respect Blogger's rate limits
      if (createdCount > 0) {
        console.log('⏳ Waiting 30 seconds to respect Blogger rate limits...');
        await new Promise(resolve => setTimeout(resolve, 30000)); // 30 seconds between posts
      } else {
        console.log('⏳ Waiting 5 seconds before next attempt...');
        await new Promise(resolve => setTimeout(resolve, 5000)); // 5 seconds if no post created
      }
    }
